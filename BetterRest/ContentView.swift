...
//  BetterRest
//
//  Created by Vishrut Jha on 5/5/24.
//
            ...
            Form {
                ...
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    Picker("Number of cups", selection: $coffeeAmount) {
                        ForEach(0..<21) { count in
                            Text("\(count) cup\(count != 1 ? "s" : "")").tag(count)
                        }
                    }.pickerStyle(WheelPickerStyle())
                        .onChange(of: coffeeAmount) { calculateBedtime() }
                }
                
                Section(header: Text("Your ideal bedtime").font(.headline)) {
                    Text(bedtime)
                        .font(.title)
                        .bold()
                }
...
//            alertTitle = "Ideally, you should sleep at:"
